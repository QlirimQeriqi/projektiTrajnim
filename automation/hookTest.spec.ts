...
test('Navigate to Home', async ({ page }) => {
  ...
  //Assert
  await expect(page.getByText('Now Showing').nth(0)).toBeVisible();
});
...
test('Navigate to My Plan', async ({ page }) => {
  ...
  //Assert
  await expect(page.getByText('My Watch Plan').nth(0)).toBeVisible();
});
...
test('Navigate to History', async ({ page }) => {
  ...
  await expect(page.getByText('Order History').nth(0)).toBeVisible();
  });
})